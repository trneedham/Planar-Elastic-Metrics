function [dist,rho_geod,theta1,theta2i,theta2n,transform_geod,c_geod,gamI]=geodesic_dist_with_seeds(b,c1,c2,num_samp,num_steps,figures)

n=num_samp; % Common number of points to resample the curves with.
...
% This step perturbs the last points of the curves--if the first and
...
num_seeds = 50; % Gives a rough search of optimal seed points and returns the best few of them.
seed_list = best_seeds_fast(c1,c2,b,num_seeds);

% We will iterate through the seeds and keep the shift with the smallest
% dist. Initialize the distance

dist = inf;

for j=1:num_seeds
    c2Shifted = ShiftF(c2,seed_list(j));
    
    %Apply Fb Transform.
    v1=curveDeriv(c1); %Find derivative curves.
    v2=curveDeriv(c2Shifted);
    ...
    [distNew,transform_geodNew]=Geod_Planar(p1,p2,b,num_steps);
    
    if distNew < dist
        dist = distNew;
        transform_geod = transform_geodNew;
    end
end
